...
// AttachedToView
void
TransformBox::AttachedToView(StateView* view)
{
...
// DetachedFromView
void
TransformBox::DetachedFromView(StateView* view)
{
	view->Invalidate(Bounds().InsetByCopy(-INSET, -INSET));
}
...
// Set
void
TransformBox::Set(const AffineTransform& t)
{
	SetTransform(t);
...
TransformBox::OffsetPivot(BPoint offset)
{
	if (offset != BPoint(0.0, 0.0)) {
		SetTransformation(Pivot() + offset,
						  Translation(),
						  LocalRotation(),
						  LocalXScale(),
						  LocalYScale());
...
TransformBox::_UpdateAffine()
{
	...
	// use this when you modified the matrix directly
	// and need to extract the affine parameters again,
	// this cannot work with sheared transformations
	// though!
	// assumes the pivot stayed fixed when the
	// matrix was modified

	// TODO: this is (probably still) broken with
	// regards to filtering the pivot!

	// extract pivot
	multiply(agg::trans_affine_translation(-fPivot.x,
										   -fPivot.y));
	...
	// rebuild the matrix from the affine parameters to be safe
	_UpdateMatrix();
}
...
TransformBox::_DragStateFor(BPoint where, float canvasZoom)
{
	...
	if (point_point_distance(where, Origin()) < inset)
