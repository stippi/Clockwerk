...
// SetFont
FontCacheEntry*
FontCache::FontCacheEntryFor(const Font& font, bool forceOutline)
{
	char signature[512];
	FontCacheEntry::GenerateSignature(signature, font, forceOutline);
	...
	if (!entry) {
		...
		if (!entry || !entry->Init(font, forceOutline)
