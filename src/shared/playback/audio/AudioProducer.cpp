...
{
	TRACE("AudioProducer::Disconnect()\n");
	...
	TRACE("AudioProducer::Disconnect() done\n");
}
...
{
	ERROR("AudioProducer::LateNoticeReceived(%lld, %lld)\n", howMuch,
		performanceTime);
...
void
AudioProducer::SetPeakListener(const BMessenger& messenger)
{
printf("AudioProducer::SetPeakListener()\n");
	delete fPeakListener;
	fPeakListener = new (std::nothrow) BMessenger(messenger);
}
...
AudioProducer::_FillNextBuffer(bigtime_t eventTime)
{
	...
	if (fPeakListener
		...
		status_t ret = fPeakListener->SendMessage(&message);
		if (ret < B_OK) {
			fprintf(stderr, "AudioProducer: problem sending peak "
				"notification: %s\n", strerror(ret));
			delete fPeakListener;
			fPeakListener = NULL;
		}
