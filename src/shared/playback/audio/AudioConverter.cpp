...
// constructor
AudioConverter::AudioConverter(AudioReader* source, uint32 format,
							   uint32 byte_order)
	: AudioReader(),
	  fSource(NULL)
{
...
// Read
status_t
AudioConverter::Read(void* buffer, int64 pos, int64 frames)
{
ldebug("AudioConverter::Read(%p, %Ld, %Ld)\n", buffer, pos, frames);
	status_t error = InitCheck();
	if (error != B_OK)
{
ldebug("AudioConverter::Read() done 1\n");
		return error;
}
	pos += fOutOffset;
...
ldebug("AudioConverter::Read() done 2\n");
...
ldebug("  source format: %s\n", formatString);
ldebug("  in format : format: %lx, sample size: %ld, channels: %ld, byte order: %lu\n",
fSource->Format().u.raw_audio.format, inSampleSize, channelCount,
fSource->Format().u.raw_audio.byte_order);
ldebug("  out format: format: %lx, sample size: %ld, channels: %ld, byte order: %lu\n",
fFormat.u.raw_audio.format, outSampleSize, channelCount,
fFormat.u.raw_audio.byte_order);
	if (inSampleSize != outSampleSize) {
...
ldebug("AudioConverter::Read() done\n");
	return B_OK;
}

// InitCheck
status_t
AudioConverter::InitCheck() const
{
