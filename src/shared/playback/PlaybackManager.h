...
class PlaybackManager : public BLooper,
						public PlaybackManagerInterface {
 private:
			struct PlayingState;
			struct SpeedInfo;
...
//			void				Step(int32 howManyFrames);
	...
	virtual	BRect				MovieBounds() const = 0;
	...
	virtual	void				SetCurrentAudioTime(bigtime_t time);
			...
			void				TriggerPlayModeChanged(int32 mode) const;
			void				TriggerLoopModeChanged(int32 mode) const;
			void				TriggerLoopingEnabledChanged(
									bool enabled) const;
			void				TriggerMovieBoundsChanged(BRect bounds) const;
			void				TriggerFPSChanged(float fps) const;
			void				TriggerCurrentFrameChanged(int32 frame) const;
			void				TriggerSpeedChanged(float speed) const;
			void				TriggerFrameDropped() const;
...
//			void				_LockPostProcessing();
 ...
 protected:
//			XSheet*				fXSheet;
//			XSheetVisualModel*	fXSheetVisualModel;
 private:
			BList				fStates;
			BList				fSpeeds;
	...
	volatile bigtime_t			fStopPlayingFrame;	// frame at which playing
			...
			BList				fListeners;
//			XSheetVisualLOAdapter* fVisualLOAdapter;
};
