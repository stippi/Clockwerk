...
// copy constructor
OptionProperty::OptionProperty(const OptionProperty& other, bool deep)
	: Property(other, deep),
	  fOptions(4),
	  fCurrentOptionID(other.fCurrentOptionID)
{
	if (!deep)
		return;
	// clone the actual options in "deep" mode
...
// Clone
Property*
OptionProperty::Clone(bool deep) const
{
	return new (nothrow) OptionProperty(*this, deep);
}
...
// Equals
bool
OptionProperty::Equals(const Property* other) const
{
	const OptionProperty* o = dynamic_cast<const OptionProperty*>(other);
	if (o) {
		return fCurrentOptionID == o->fCurrentOptionID;
	}
	return false;
}
...
// SetValue
bool
OptionProperty::SetValue(int32 id)
{
