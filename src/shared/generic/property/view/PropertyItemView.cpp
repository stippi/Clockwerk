...
{
	float width = 0.0;
	if (const ::Property* property = Property())
...
// UpdateLowColor
void
PropertyItemView::UpdateLowColor()
{
	_UpdateLowColor();
}
...
// Property
::Property*
PropertyItemView::Property() const
{
	if (fEditorView)
...
// UpdateObject
void
PropertyItemView::UpdateObject(::Property* property)
{
	if (fParent)
		fParent->UpdateObject(property);
}
...
PropertyItemView::_UpdateLowColor()
{
	...
	const ::Property* p = fEditorView ? fEditorView->GetProperty() : NULL;
	if (p && p->Animator() && p->Animator()->CountKeyFrames() > 0)
		lowColor = fSelected ? kDarkOrange : kLightOrange;
	else {
		lowColor = fParent ? fParent->LowColor() : kWhite;
		if (fSelected)
