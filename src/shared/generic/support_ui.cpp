...
void
center_frame_on_screen(BRect& frame, BWindow* window)
{
	if (!frame.IsValid())
		return;
	...
	float left = floorf(screenFrame.left
					+ (screenFrame.Width() - frame.Width()) / 2.0);
	float top = floorf(screenFrame.top
					+ (screenFrame.Height() - frame.Height()) / 2.0);
...
plain_font_height()
{
	...
	return ceilf((ceilf(fh.ascent) + ceilf(fh.descent)) * 1.2);
}

// show_popup_menu
void
show_popup_menu(BPopUpMenu* menu, BPoint where, BView* fromView, bool keepOpen)
{
	menu->SetAsyncAutoDestruct(true);
	menu->SetFont(be_plain_font);

	where = fromView->ConvertToScreen(where);
	if (keepOpen) {
