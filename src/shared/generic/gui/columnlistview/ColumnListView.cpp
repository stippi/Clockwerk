...
ColumnListView::ColumnListView()
	: BView(BRect(0.0, 0.0, 100.0, 100.0), NULL, B_FOLLOW_NONE,
	  ...
	  fDragMode(CLV_DRAG_MODE_BITMAP),
	  fLastClickedIndex(-1),
...
ColumnListView::AttachedToWindow()
{
	...
	if (ScrollView* scrollView = dynamic_cast<ScrollView*>(ScrollSource())) {
		float defaultItemHeight = plain_font_height();
		scrollView->SetSmallSteps(defaultItemHeight, defaultItemHeight);
	}
}
...
ColumnListView::MessageReceived(BMessage* message)
{
	switch (message->what) {
		case B_MOUSE_WHEEL_CHANGED: {
			ScrollView* scrollView = dynamic_cast<ScrollView*>(ScrollSource());
			if (!fScrollingEnabled || !scrollView)
				break;
			float x;
			float y;
			...
			scrollOffset.x += x * scrollView->HSmallStep() * 3;
			scrollOffset.y += y * scrollView->VSmallStep() * 3;
...
// ItemAtFast
ColumnListItem*
ColumnListView::ItemAtFast(int32 index) const
{
	return (ColumnListItem*)fItems.ItemAtFast(index);
}
...
// MakeDragMessage
bool
ColumnListView::MakeDragMessage(BMessage* message,
	Column** column1, Column** column2)
{
	// dragging items not supported by default
	return false;
}
...
ColumnListView::InitiateDrag(BPoint point, int32 index, bool wasSelected)
{
	...
	if (!MakeDragMessage(&message, &column1, &column2))
		return false;
	...
	bool fade = false;
	for (numItems = 0;
		 ColumnListItem *item = ItemAt(CurrentSelection(numItems));
		 numItems++) {
	...
	// drag either bitmap or dotted rect
	BBitmap* dragBitmap = NULL;
	if (fDragMode == CLV_DRAG_MODE_BITMAP)
		dragBitmap = new BBitmap(dragRect, B_RGB32, true);
	bool success = false;
	if (dragBitmap && dragBitmap->IsValid()) {
		BView *v = new BView(dragRect, NULL, B_FOLLOW_NONE, B_WILL_DRAW);
		if (dragBitmap->Lock()) {
			dragBitmap->AddChild(v);
			BRect itemBounds(dragRect);
			itemBounds.bottom = 0.0;
			// let all selected items, that fit into our drag_bitmap, draw
			int32 firstColumn = _VisibleColumnIndexOf(dragRect.left);
			int32 lastColumn = _VisibleColumnIndexOf(dragRect.right);
			for (int32 i = 0; i < numItems; i++) {
				...
				if (column1 || column2) {
					// draw the specified columns
					BRect columnRect(itemBounds);
					if (column1) {
						columnRect.right = columnRect.left + column1->Width();
						item->Draw(v, column1, columnRect, columnRect,
							item->Flags(), &Colors()->item_colors);
						columnRect.OffsetBy(column1->Width(), 0);
					}
					if (column2) {
						columnRect.right = columnRect.left + column2->Width();
						item->Draw(v, column2, columnRect, columnRect,
							item->Flags(), &Colors()->item_colors);
					}
				} else {
					...
					for (int32 c = firstColumn; c <= lastColumn; c++) {
						Column* column = _VisibleColumnAt(c);
						BRect columnRect(_VisibleColumnFrame(column));
						item->Draw(v, column, columnRect, columnRect,
							item->Flags(), &fColors->item_colors);
					}
				}
				itemBounds.top = itemBounds.bottom + 1.0;
			}
			// dark frame around everything
			v->SetHighColor(Colors()->item_colors.foreground);
			v->StrokeRect(dragRect);
			v->Sync();
	...
	if (success)
		DragMessage(&message, dragBitmap, B_OP_ALPHA, BPoint(0.0, 0.0), this);
	else {
		delete dragBitmap;
...
// IsSortingColumn
bool
ColumnListView::IsSortingColumn(int32 columnIndex) const
{
	bool isSortingColumn = false;
	if (fSecondarySortColumn) {
		if (ColumnIndexOf(fSecondarySortColumn) == columnIndex)
			isSortingColumn = true;
	}
	if (fPrimarySortColumn) {
		if (ColumnIndexOf(fPrimarySortColumn) == columnIndex)
			isSortingColumn = true;
	}
	return isSortingColumn;
}
...
ColumnListView::_ActualItemsRect() const
{
	...
	if (_CountVisibleColumns() == 0 || CountItems() == 0) {
		...
		rect.right = std::min(rect.right,
			_VisibleColumnFrame(lastColumn).right);
