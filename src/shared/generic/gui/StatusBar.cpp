...
static const bigtime_t kTickPeriod = 40000;
static const bigtime_t kTextDecayDelay = 10 * 1000 * 1000;
static const bigtime_t kTextDecayTime = 3 * 1000 * 1000;
static const int32 kBusyStripeThickness = 10;
static const float kMinScrollBarWidth = 6 * B_V_SCROLL_BAR_WIDTH;
...
StatusBar::AttachedToWindow()
{
	...
	if (fFadePulse && fFadePulse->InitCheck() != B_OK) {
		...
		fFadePulse = new BMessageRunner(BMessenger(this), &message, kTickPeriod);
	}
	if (fBusyPulse && fBusyPulse->InitCheck() != B_OK) {
		delete fBusyPulse;
		BMessage message(MSG_BUSY_PULSE);
		fBusyPulse = new BMessageRunner(BMessenger(this), &message, kTickPeriod);
	}
}
...
/*
	...
	minWidth = width + kMinScrollBarWidth;
...
// GetPreferredSize
void
StatusBar::GetPreferredSize(float* width, float* height)
{
	font_height fontHeight;
	GetFontHeight(&fontHeight);
	float labelHeight = ceil(fontHeight.ascent) + ceilf(fontHeight.descent);
	...
	if (width != NULL)
		*width = borderWidth + 10.0 + 25.0 + 10.0 + borderWidth;
	if (height != NULL)
		*height = borderWidth + 2.0 + labelHeight + 2.0 + borderWidth;
}
...
// MessageReceived
void
StatusBar::MessageReceived(BMessage* message)
{
	switch (message->what) {
		case MSG_FADE_PULSE:
			_FadeTick();
...
StatusBar::MouseMoved(BPoint point, uint32 transit, const BMessage *msg)
{
	...
	if (fScrollBar != NULL
		&& ((fScrollBar->Bounds().Width() - x) <= kMinScrollBarWidth)) {
		return;
	}

	// TODO: This should be reimplemented using layout mangement. It isn't
	// currently used in Clockwerk, WonderBrush or eXposer.
	ResizeBy(x, 0.0);
	BRect r(Bounds());
	r.left = r.right - 10.0;
	fDirty = true;
	...
	if (fJobProgress >= 0.0 || fBusyStep >= 0) {
		...
		if (fJobProgress > 0.0) {
			...
			for (float y = b.top; y <= b.bottom; y++) {
				...
				for (int32 stripe = -1; stripe * kBusyStripeThickness <= b.Width() + 1; stripe++) {
					// x coordinates for stripe
					float x1 = x + stripe * kBusyStripeThickness * 2;
					float x2 = x + stripe * kBusyStripeThickness * 2
							   + kBusyStripeThickness - 1;
					// check bounds
					if (x1 < b.left) {
						x1 = b.left;
						if (x2 < x1)
...
StatusBar::_FadeTick()
{
	if (fOpacity > 0.0) {
		if (fDecayDelay > 0) {
			fDecayDelay -= kTickPeriod;
			return;
		}
		float steps = (float)kTextDecayTime / (float)kTickPeriod;
		fOpacity -= (1.0 / steps);
		if (fOpacity < 0.001)
			fOpacity = 0.0;
...
StatusBar::_BusyTick()
{
	if (fBusyPulse) {
		fBusyStep++;
		if (fBusyStep >= kBusyStripeThickness * 2)
			fBusyStep = 0;
		fDirty = true;
		Invalidate();
	}
