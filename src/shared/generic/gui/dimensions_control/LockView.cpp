	...
	fClosedDisabledBitmap(NULL)
{
	...
	memcpy(fOpenEnabledBitmap->Bits(), kOpenEnabledBits,
		fOpenEnabledBitmap->BitsLength());
	memcpy(fOpenDisabledBitmap->Bits(), kOpenDisabledBits,
		fOpenDisabledBitmap->BitsLength());
	memcpy(fClosedEnabledBitmap->Bits(), kClosedEnabledBits,
		fClosedEnabledBitmap->BitsLength());
	memcpy(fClosedDisabledBitmap->Bits(), kClosedDisabledBits,
		fClosedDisabledBitmap->BitsLength());
}
...
LockView::Draw(BRect update)
{
	...
	BeginLineArray(8);
		...
		AddLine(BPoint(right - 1.0, top + 2.0),
				BPoint(right - 1.0, middle - (kImageWidth / 2.0 + room)),
					shadow);
...
LockView::MinSize()
{
	return BSize(kImageWidth, kImageHeight);
}
