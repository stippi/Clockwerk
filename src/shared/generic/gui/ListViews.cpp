...
DragSortableListView::DragSortableListView(BRect frame, const char* name,
	...
	: BListView(frame, name, type, resizingMode, flags),
	  ...
	  fMouseWheelFilter(new (nothrow) MouseWheelFilter(this)),
	  fScrollPulse(NULL),
	  fDropIndex(-1),
...
DragSortableListView::InitiateDrag(BPoint point, int32 index, bool)
{
	...
	if (item) {
		...
		bool fade = false;
		for (numItems = 0;
			BListItem* item = ItemAt(CurrentSelection(numItems)); numItems++) {
		...
		BBitmap* dragBitmap = new (nothrow) BBitmap(dragRect, B_RGB32, true);
		if (dragBitmap && dragBitmap->IsValid()) {
			if (BView *v = new (nothrow) BView(dragBitmap->Bounds(), "helper",
		...
		switch (message->what) {
			case MSG_TICK: {
				...
				if (scrollV != 0.0 && fScrollView) {
					if (BScrollBar* scrollBar = fScrollView->ScrollBar(
							B_VERTICAL)) {
